...
class PostgresBackend(StorageBackend):
    ...
    # ---------------------------------------------------------------- handoff
    async def handoff_save(self, key, value, *, source_surface=None, event_id=None) -> dict:
        return await self._append(HANDOFF_NS, key, value, "handoff", [], source_surface, event_id, False)

    async def handoff_load(self, key) -> dict | None:
        return await self.memory_get(HANDOFF_NS, key)

    async def handoff_list(self, *, limit=100) -> list[dict]:
        return await self.memory_list(HANDOFF_NS, kind="handoff", limit=limit)

    # --------------------------------------------------------------- sessions
    async def session_create(self, *, surface=None, metadata=None) -> dict:
        sid = uuid.uuid4()
        async with self.pool.connection() as conn:
            conn.row_factory = dict_row
            cur = await conn.execute(
                "INSERT INTO session (session_id, surface, metadata) VALUES (%s, %s, %s) RETURNING *",
                (sid, surface, Jsonb(metadata or {})),
            )
            row = await cur.fetchone()
        return {
    ...
    async def session_append_event(self, session_id, kind, payload) -> dict:
        last_exc: Exception | None = None
        for _ in range(_MAX_RETRIES):
            try:
                async with self.pool.connection() as conn:
                    ...
                    async with conn.transaction():
                        cur = await conn.execute(
                            """
                            INSERT INTO session_event (session_id, seq, kind, payload)
                            SELECT %s, COALESCE(MAX(seq), 0) + 1, %s, %s
                            FROM session_event WHERE session_id = %s
                            RETURNING *
                            """,
                            (session_id, kind, Jsonb(sanitize(payload)), session_id),
                        )
                        row = await cur.fetchone()
                return {
                    "session_id": str(row["session_id"]),
    ...
    async def session_get(self, session_id) -> dict | None:
        async with self.pool.connection() as conn:
            conn.row_factory = dict_row
            cur = await conn.execute("SELECT * FROM session WHERE session_id = %s", (session_id,))
            row = await cur.fetchone()
        if row is None:
            return None
        return {
    ...
    async def session_list(self, *, limit=50) -> list[dict]:
        async with self.pool.connection() as conn:
            conn.row_factory = dict_row
            cur = await conn.execute(
                "SELECT * FROM session ORDER BY created_at DESC LIMIT %s", (limit,)
            )
            rows = await cur.fetchall()
        return [
            {
    ...
    async def session_events(self, session_id, *, limit=200) -> list[dict]:
        async with self.pool.connection() as conn:
            conn.row_factory = dict_row
            cur = await conn.execute(
                "SELECT * FROM session_event WHERE session_id = %s ORDER BY seq ASC LIMIT %s",
                (session_id, limit),
            )
            rows = await cur.fetchall()
        return [
    ...
    async def artifact_read_range(self, sha256, offset: int, length: int) -> bytes | None:
        ...
        async with self.pool.connection() as conn:
            cur = await conn.execute(
                "SELECT substring(bytes FROM %s FOR %s) FROM artifact WHERE sha256 = %s",
