"""FastAPI application: owns the ONE connection pool, mounts the MCP app, and
exposes /healthz and a streamed /artifact/{sha256}.

Boot discipline (addendum #1): the lifespan never waits unbounded. The pool is
built with explicit timeouts, opened immediately, and a single SELECT 1
readiness probe is wrapped in asyncio.timeout(). On failure we raise — a
terminal crash lets the Reserved VM restart instead of hanging forever.
"""
...
class MCPAuthMiddleware(BaseHTTPMiddleware):
    """Bearer-token gate for the /mcp surface. /healthz stays open (liveness probe)."""
