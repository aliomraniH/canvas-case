...
async def test_memory_search_finds_substring(backend, ns):
    await backend.memory_save(ns, "k", {"note": "titration schedule"})
    hits = await backend.memory_search("titration", namespace=ns)
    assert any("titration" in str(h["value"]) for h in hits)


async def test_handoff_round_trip_across_surfaces(backend):
    key = f"handoff-{base64.b16encode(b'x').decode()}"
    await backend.handoff_save(key, {"summary": "did X"}, source_surface="web")
    loaded = await backend.handoff_load(key)
    assert "did X" in str(loaded["value"])
    assert loaded["source_surface"] == "web"


async def test_session_events_in_order(backend):
    s = await backend.session_create(surface="cli")
    sid = s["session_id"]
    await backend.session_append_event(sid, "start", {"n": 1})
    await backend.session_append_event(sid, "step", {"n": 2})
    evs = await backend.session_events(sid)
    assert [e["seq"] for e in evs] == [1, 2]
    assert (await backend.session_get(sid))["surface"] == "cli"
